...
class CreateOrderService {
  ...
  public async execute({ customer_id, products }: IRequest): Promise<Order> {
    ...
    if (!customerExists) {
      throw new AppError('Could nor find any customer with the gicen id');
    }
    ...
    if (findProductWithNoQuantityAvailable.length) {
      throw new AppError(
        `The quantity ${findProductWithNoQuantityAvailable[0].quantity} is not avilable for ${findProductWithNoQuantityAvailable[0].id}`,
      );
    }
